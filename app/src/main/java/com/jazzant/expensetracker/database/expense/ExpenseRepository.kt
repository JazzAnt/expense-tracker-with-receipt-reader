...
class ExpenseRepository(private val expenseDao: ExpenseDao) {
  ...
  fun getAllExpenses(dateRange: Pair<Long, Long>) =
    expenseDao.getAllExpenses(startDate = dateRange.first, endDate = dateRange.second)

  fun getAllExpenses(category: String, dateRange: Pair<Long, Long>) = expenseDao.getAllExpenses(
    category = category,
  ...
  fun getSumOfExpenses(dateRange: Pair<Long, Long>) =
    expenseDao.getSumOfExpenses(startDate = dateRange.first, endDate = dateRange.second)

  fun getSumOfExpenses(category: String, dateRange: Pair<Long, Long>) = expenseDao.getSumOfExpenses(
    category = category,
    startDate = dateRange.first,
    endDate = dateRange.second
  )
